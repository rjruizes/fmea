<script lang="ts">
  ...
  function calculateRiskAssessment(rpn:number, severity:number, occurrence:number, detection:number) {
    const sev = severity
    if (rpn < 10 && sev <5) {
      return cases[0]
    }
  ...
  $: selectedCase = calculateRiskAssessment(rpn, severity, occurrence, detection)
</script>
